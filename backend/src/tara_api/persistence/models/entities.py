...
from sqlalchemy import JSON, Boolean, Enum, ForeignKey, Index, Integer, LargeBinary, String, Text, UniqueConstraint, Uuid
...
class TaskExecutionPayloadModel(Base):
    """One encrypted, task-bound input envelope; plaintext never enters this schema."""

    __tablename__ = "task_execution_payloads"

    id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid4)
    task_id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("scheduled_tasks.id", ondelete="CASCADE"), unique=True, nullable=False)
    owner_id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("owners.id", ondelete="CASCADE"), nullable=False)
