...
- Validation: task tests passed (31); `python -m ruff check . --no-cache` passed; and `python -m mypy src --cache-dir .mypy_cache_run --no-incremental` passed (112 source files). The only output warning remains the pre-existing upstream `StarletteDeprecationWarning` from FastAPI's `TestClient`.
