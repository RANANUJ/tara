...
- Cleanup recovery increment: terminal one-time task payloads are now removed only after the payload retention cutoff while active, claimed, recurring, and pending-confirmation payloads remain untouched. Before every due claim pass, expired active-task leases are recovered in a transaction: the old claimed/running run receives the safe `task_claim_lease_expired` terminal code and only then can the still-active enabled task become eligible again. Cleanup uses one global payload batch budget across inactive and completed candidates.
- Shutdown increment: the single process-local poller now uses validated `TARA_SCHEDULER_SHUTDOWN_TIMEOUT_SECONDS` (1..60, default 10) when awaiting cancellation. Start remains idempotent; stop remains idempotent and bounds the wait for polling, cleanup, and child execution work that is part of the poll task. Interrupted claims are not marked successful and recover only through the persisted lease protocol after restart.
- Poll concurrency increment: `ScheduledTaskScheduler.tick` is serialized by a same-process asynchronous lock around stale-lease recovery, due claiming, and execution dispatch. Ten concurrent ticks against one due task now produce exactly one claim/run path; later ticks observe no eligible duplicate. SQLite/database conditional claiming remains the authority across process boundaries, while deployment continues to require exactly one scheduler leader.
- Claim/permit ordering increment: each poll computes a dispatch bound of `min(TARA_SCHEDULER_DUE_BATCH_SIZE, TARA_SCHEDULER_MAX_CONCURRENT_RUNS)` before lease recovery and due claiming. A poll therefore cannot create more claimed-but-waiting tasks than the global execution capacity; remaining due work remains unclaimed for later ticks, cancellation, or ordinary lease-safe recovery.
- Claim revalidation increment: after a task has acquired global and owner permits, the scheduler reloads it through an owner-scoped `task_id + claim_id` query requiring active/enabled state before touching the encrypted payload or moving the run to `running`. Cancellation, pause/disable, deletion, or cleared claim that commits while the task is waiting for a permit therefore prevents stale execution instead of relying on the original in-memory candidate.
- Global-cap test increment: a canonical-registry task test now creates two independently encrypted due tasks with `maximum_concurrency=1`, verifies exactly one central executor invocation in the first poll, and verifies the remaining task is unclaimed and executes only in the following poll. This confirms the dispatch bound leaves excess due work eligible rather than creating claimed backlog.
- Owner-limiter lifecycle increment: per-owner semaphores are now reference-counted under an async guard. Every claimed task retains its persisted owner’s limiter before waiting and releases it in `finally` after success, failure, timeout, cancellation, or stale revalidation rejection; when the final user exits, the owner entry is removed. This preserves owner isolation without allowing the limiter map to grow indefinitely.
- Owner-limiter test increment: deterministic task coverage retains then releases the limiter for the canonical authenticated owner and verifies the owner-keyed entry is removed. Tara remains a documented single-owner v1 deployment, so cross-owner fairness is not an applicable product guarantee; the persisted owner ID still remains the sole limiter identity and is never client-supplied.
- Focused task coverage: `backend/tests/tasks/test_task_service.py` now verifies read-only direct activation, registered/disabled/invalid capability rejection, safe hashes, consequential proposal owner/session/hash binding, expiry attachment, idempotent proposal reuse, no raw persistence/audit payload, safe proposal/attachment failures, owner/session M14 approval, exactly-once consumption, activation with a computed next run, replay rejection, persisted binding mismatch rejection, instruction invalidation, ten-way equivalent consequential creation, mismatched-payload creation race rejection, session isolation, two-way approval activation race safety, cancellation before poll, cancellation after claim but before execution, cancellation while capability execution is blocked, repeated cancellation idempotency, foreign-owner cancellation rejection, timeout with exactly one `task_execution_timed_out` terminal outcome, late completion state isolation after timeout and cancellation, global and per-owner limiter release after timeout/cancellation, payload revocation/replacement while claimed preventing execution, pre-execution invalidation (pause, disable, delete, binding mismatch), non-writing of recurring `next_run_at` after invalidation/cancellation, and scheduler usability after timeout/cancellation.
- Files changed in this increment: `backend/tests/tasks/test_task_service.py` and `docs/IMPLEMENTATION_STATUS.md`.
- Validation: all task tests passed (`python -m pytest backend/tests/tasks` passed 45/45); M14 confirmation tests passed (`python -m pytest backend/tests/auth/test_confirmation_session_binding.py backend/tests/test_safety.py` passed 13/13); capability/policy tests passed (`python -m pytest backend/tests/capabilities` passed 5/5); migration/readiness tests passed (`python -m pytest backend/tests/test_migrations.py backend/tests/test_readiness.py` passed 2/2); `python -m ruff check backend` passed; `python -m mypy backend/src` passed (112 source files); and full backend test suite passed (`python -m pytest backend/tests` passed 282/282). The only output warning remains the pre-existing upstream `StarletteDeprecationWarning` from FastAPI's `TestClient`.
- Remaining M16 work: orphan/owner-deletion cleanup tests; full lifecycle/retry policy completion; fresh proposal creation after invalidation; REST/WebSocket transport; frontend task management; scheduler health/status; final documentation; and acceptance. M16 is not complete and no M17 work has started.
