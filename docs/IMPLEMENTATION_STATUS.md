...
- Focused typed-create/proposal coverage: `backend/tests/tasks/test_task_service.py` creates an isolated persisted owner/session through the existing authentication store and uses canonical registry capabilities. It verifies read-only direct activation, registered/disabled/invalid capability rejection, safe hashes, consequential proposal owner/session/hash binding, expiry attachment, idempotent proposal reuse, no raw persistence/audit payload, and safe proposal/attachment failures.
- Files changed in this increment: `backend/src/tara_api/capabilities/registry.py`, `backend/src/tara_api/domain/protocols.py`, `backend/src/tara_api/tasks/mapping.py`, `backend/src/tara_api/tasks/service.py`, `backend/src/tara_api/persistence/repositories/tasks.py`, `backend/src/tara_api/observability/health.py`, `backend/tests/tasks/test_task_service.py`, and `docs/IMPLEMENTATION_STATUS.md` (alongside the prior M16 task/domain/UoW files already in progress).
- Validation: `python -m pytest backend/tests/tasks -q` passed (13); M14 confirmation, capability, and context-policy tests passed (8); migration/startup/readiness tests passed (7); `python -m ruff check backend` passed; `python -m mypy backend/src` passed (110 source files); and `python -m pytest backend/tests -q` passed (250). The only output warning remains the pre-existing upstream `StarletteDeprecationWarning` from FastAPI's `TestClient`.
- Remaining M16 work: confirmation approval, one-time consumption, and task activation; confirmation invalidation after bound-field updates; concurrency-safe database create races; bounded orphan-proposal cleanup; scheduler runtime; REST/WebSocket transport; frontend task management; health/status; final documentation; and acceptance. M16 is not complete and no M17 work has started.
