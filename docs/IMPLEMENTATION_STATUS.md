...
- Validation: task, M14 confirmation, capability, context-policy, migration, startup, and readiness tests passed (42); `python -m ruff check backend` passed; `python -m mypy backend/src` passed (110 source files); and `python -m pytest backend/tests -q` passed (261). The only output warning remains the pre-existing upstream `StarletteDeprecationWarning` from FastAPI's `TestClient`.
- Remaining M16 work: typed updates for currently immutable capability/target/parameter fields, fresh proposal creation after invalidation, bounded orphan-proposal cleanup, scheduler runtime and due-task claiming, REST/WebSocket transport, frontend task management, scheduler health/status, final documentation, and acceptance. M16 is not complete and no M17 work has started.
