...
- Focused typed-create coverage: `backend/tests/tasks/test_task_service.py` now creates an isolated persisted owner/session through the existing authentication store and uses the configured allowlisted filesystem tool through the canonical registry. It proves registered capability creation and safe metadata persistence, equivalent idempotency, and rejection of unknown, disabled, or schema-invalid capabilities. It makes no proposal and performs no tool execution, network call, or provider call.
- Files changed in this increment: `backend/src/tara_api/domain/tasks.py`, `backend/src/tara_api/tasks/mapping.py`, `backend/src/tara_api/tasks/service.py`, `backend/src/tara_api/persistence/unit_of_work.py`, `backend/tests/tasks/test_task_service.py`, and `docs/IMPLEMENTATION_STATUS.md`.
- Validation: `python -m pytest backend/tests/tasks/test_task_service.py backend/tests/tasks/test_task_schedule_validation.py backend/tests/capabilities -q` passed (15); `python -m ruff check backend` passed; `python -m mypy backend/src` passed (110 source files). The only output warning remains the pre-existing upstream `StarletteDeprecationWarning` from FastAPI's `TestClient`.
- Remaining M16 work: remaining typed-create callers and fixtures; M14 proposal creation and attachment; confirmation approval, consumption, activation, and invalidation; concurrency-safe creation races; scheduler runtime; REST/WebSocket transport; frontend task management; health/status; security documentation; and final acceptance. M16 is not complete and no M17 work has started.
