...
## 26. M11A Foreground Wake-Word Boundary
...
- Detection is disabled by default. The only implementation is a deterministic offline fake restricted to development/test. No production wake-word engine, model path, model download, cloud fallback, microphone activation, WebSocket event, status integration, or background runtime is added.
