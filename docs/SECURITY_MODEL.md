...
## 23. M10A TTS Provider Boundary

- M10A accepts server-created immutable synthesis identity and normalized plain text only. It rejects empty/oversized/NUL/invalid-Unicode text, raw SSML, client provider arguments, client filesystem paths, and client-controlled owner/session identity.
- Only final, bounded mono PCM signed 16-bit little-endian results are represented. Result validation enforces sample-rate/channel/bit-depth allowlists, byte bounds, frame alignment, duration/sample consistency, ordered chunks when present, and one complete terminal result.
- Piper runs only an explicitly configured local executable with a manually provisioned voice model. The adapter uses argument arrays and stdin, never a shell; it does not download models, use cloud fallback, retain temporary audio, or expose model paths, stderr, process output, or exit details. Cancellation terminates and reaps an active child process.
- ElevenLabs is an explicitly configured optional cloud adapter, never default or fallback. Its API key is server-only and redacted; standard tests use mocked HTTP only. M10A has no WebSocket delivery, playback, or UI indicator, so production cloud enablement remains subject to a later visible privacy disclosure.
- TTS health is an internal bounded snapshot only. It performs no synthesis, audio generation, model download, or global readiness/status integration. Normal logs contain safe IDs, provider/voice labels, state, duration, byte count, and code only; they exclude text, generated audio, tokens, keys, paths, stderr, and provider exceptions.

## 24. M10B TTS Service, Queue, and Retention Boundary

- M10B resolves synthesis input only through a server-side completed-agent-response source port. The service command contains a source agent request reference plus voice/language/format only; callers cannot provide text, owner, session, connection, provider, or idempotency identity. Source resolution rechecks owner, authenticated session, optional connection, agent request, and optional assistant-turn binding before work is registered.
- A process-local FIFO registry enforces global queue/concurrency and per-owner, per-session, and per-connection limits. It is explicitly unsuitable for multi-process deployment. Derived idempotency hashes bind the approved response identity, configured provider, voice, language, and output format; duplicate work cannot produce a second provider call.
- Final-only provider audio is validated before deterministic, frame-aligned post-synthesis raw-PCM chunking. Chunk metadata is ordered and contains offsets, lengths, final flag, and duration ranges. M10B does not claim provider streaming or emit WebSocket chunks, and it never exposes invalid independent WAV fragments.
- Audio bytes are transient, process-local, globally byte-bounded, and deterministically released after consumption, cancellation, timeout, terminal expiry, eviction, or shutdown. M10B adds no SQLite audio/text persistence, object storage, temporary audio files, or schema migration. Terminal records retain content-minimized metadata only.
- Cancellation is owner/session/connection-bound and non-enumerating. Disconnect, session invalidation, and shutdown seams cancel matching queued or active work and suppress late results. Logs retain only safe identifiers, provider/voice labels, states, sizes, counts, durations, and stable codes; they exclude source text, audio/chunks, credentials, tokens, paths, stderr, and provider exceptions.

## 25. M10C TTS Transport and Foreground Playback Boundary

- TTS starts only from a server-resolved completed successful agent response bound to the originating owner, session, connection, and conversation. Clients may only issue `tts.cancel` with a synthesis UUID; arbitrary text and identity override fields are not protocol inputs.
- Audio delivery uses bounded base64 raw-PCM events with server-issued offset/sequence metadata. Delivery is process-local, connection-isolated, timeout-bounded, and releases retained bytes before transport completion. Disconnect and session invalidation cancel matching work; no TTS audio is persisted, logged, stored in browser storage, or sent to another connection.
- Browser playback is foreground-only and requires a user-established Web Audio interaction. It accepts only validated raw PCM metadata, rejects stale/duplicate/out-of-order chunks, has one active stream, clears buffers on cancellation/socket close/error, and exposes explicit Stop plus foreground VAD-triggered barge-in seams. Browser echo cancellation, speakers, and suspended/autoplay contexts remain environmental limits; no background, screen-off, wake-word, or native behavior is claimed.

## 26. M11 Foreground Wake-Word Boundary

- M11A accepts only server-bound owner, authenticated session, connection, and M7 audio-session identities. Foreground PCM must already be validated by the M7 audio boundary; browser-provided identity and lifecycle assertions never authorize a wake event.
- Detection is disabled by default. The only implementation is a deterministic offline fake restricted to development/test. Authenticated `wakeword.enable`/`wakeword.disable`, safe lifecycle/detected/error WebSocket events, and authenticated status/readiness reporting are available. No production wake-word engine, model path, model download, cloud fallback, microphone activation, or background runtime is added.
- Wake-word evaluation is bounded per connection/audio session, checks canonical PCM metadata, rejects stale/foreign/duplicate frames, applies threshold, consecutive-detection, debounce, and per-session cooldown rules, and removes state on audio-session, connection, or session cleanup. It cannot create agent requests, tool calls, permissions, confirmations, or actions.
- Wake-word frames and phrase audio are process-local inputs only and are never persisted or logged. Normal diagnostics may contain only a safe lifecycle state, provider label, trigger count/cooldown state, and stable code; they exclude PCM, transcript, agent text, credentials, paths, and provider exception text.
- The browser controller requires an already active, permitted microphone stream, an open visible page, an authenticated connected socket, and user enablement. It stops or suspends on hidden/suspended page state, permission revocation, device change, socket close, unload, and TTS playback. Native background, screen-off, and locked-device capabilities remain `false` and require a separately approved native bridge.
